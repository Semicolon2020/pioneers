...
public class ServiceSuivi implements IService<Suivi> {
    ...
    {
    PreparedStatement pre=con.prepareStatement("INSERT INTO `pionnersapp`.`suivi` (`nutrition`, `sommeil`, `sociabilite`, `psychologie`, `id_e`) VALUES ( ?, ?, ?, ?, ?);");
    ...
    public ObservableList<Suivi> readAllV3(String cin) throws SQLException {
    ObservableList<Suivi> Data = FXCollections.observableArrayList();
    ste=con.createStatement();
    ResultSet rs=ste.executeQuery("select v.id , nutrition,sommeil,sociabilite,psychologie,(SELECT concat(e.nom,' ',e.prenom) FROM enfant e WHERE e.cin_p = '"+cin+"' )nom  FROM suivi v ");
     while (rs.next()) {                
               int id=rs.getInt(1);
               String nutrition=rs.getString(2);
               String sommeil=rs.getString(3);
